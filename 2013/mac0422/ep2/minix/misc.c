...
#include <string.h>
#include <unistd.h>				/* acho que a função int getnprocnr(pid_t pid) está lá */
...
int ancestral (int id_do_proc) {
	int i = getnprocnr(id_do_proc);
	printf( "\n\nname: %s\npid1: %d\npid2: %d\nparent = %s\n\n", mproc[id_do_proc].mp_name, id_do_proc, mproc[id_do_proc].mp_pid, mproc[mp[id_do_proc].mp_parent].mp_pid );
}
