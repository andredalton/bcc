...
void imprimeClassificacao( int lPrint) {
   if (1) {
      if (debug)
         ...
         printf( "Classificacao %dmin:\n", 30 * (lPrint + 1));
      printf( "\t%d %f\n", tempoEspaco[lPrint][0].id,
                           tempoEspaco[lPrint][0].posicao );
   }
}
...
void *classificacao( void) {
   ...
   printf( "\t%d %d %f\n", tempoEspaco[0][0].id, tempoEspaco[0][0].ms,
                           tempoEspaco[0][0].posicao);
   return NULL;
}
...
void *atleta( Atleta a) {
   ...
   for (i = 0; i < NATAM; i++) {
      ...
      if (tempoTotal( a) / deltaTime != t / deltaTime) {
         p = 100 * i + 100 * ((tempoTotal( a) - tempoTotal( a) % deltaTime)
             - t) / (tempoTotal( a) - t);
         atualizaPosicao( &tempoEspaco[tempoTotal( a) / deltaTime][a->id],
                          a->id, (tempoTotal( a) - tempoTotal( a) % deltaTime),
                          p);
         t = tempoTotal( a);
      }
   }
   ...
   transicao( a, T1);
   if (tempoTotal( a) / deltaTime != t / deltaTime) {
      atualizaPosicao( &tempoEspaco[tempoTotal( a) / deltaTime][a->id], a->id,
                       (tempoTotal( a) - tempoTotal( a) % deltaTime),
                       100 * NATAM );
   }
   t = tempoTotal( a);
   /*******************
   ...
   for (i = 0; i < CITAM; i++) {
      ciclismo( a, PLANO);
      if (tempoTotal( a) / deltaTime != t / deltaTime) {
         p = 1000 * i + 1000 * ((tempoTotal( a) - tempoTotal( a) % deltaTime)
             - t) / (tempoTotal( a) - t);
         atualizaPosicao( &tempoEspaco[tempoTotal( a) / deltaTime][a->id],
                          a->id, (tempoTotal( a) - tempoTotal( a) % deltaTime),
                          p);
   ...
   for (i = 0; i < COTAM; i++) {
      corrida( a);
      if (tempoTotal( a) / deltaTime != t / deltaTime) {
         p = 100 * NATAM + 1000 * i + 1000 * ((tempoTotal( a)
             - tempoTotal( a) % deltaTime) - t) / (tempoTotal( a) - t);
         atualizaPosicao( &tempoEspaco[tempoTotal( a) / deltaTime][a->id],
                          a->id, (tempoTotal( a) - tempoTotal(a) % deltaTime),
                          p);
         t = tempoTotal( a);
   ...
   atualizaPosicao( &tempoEspaco[0][a->id], a->id, tempoTotal( a), 100 * NATAM
                    + 1000 * CITAM + 1000 * COTAM);
...
int ironMain( int argc, char *argv[]) {
   ...
   /* Alocando a quantidade máxima de tics necessária para guardar as
    * informações de tempo dos atletas.
    *****************************************************************/
   tempoEspaco = (PosicaoAtleta **) mallocX( (29 * debug + 1)
                 * TMAX * sizeof (PosicaoAtleta *));
   ...
   */
   ...
   for (i = 0; i < natletas; i++) {
      ...
      if (pthread_create( &(ids_atletas[i]), NULL, atleta,
          (void *) novoAtleta( MASCULINO, PROFISSIONAL, i))) {
